...
class DiscoveryRepository(private val client: InnertubeBrowseClient = InnertubeBrowseClient()) {
    ...
    suspend fun homeFeed(): List<HomeSection> {
        ...
        return withContext(Dispatchers.IO) {
            coroutineScope {
                val home = async { runCatching { BrowseParser.parseHomeFeed(client.browse(InnertubeBrowseClient.HOME_BROWSE_ID)) }.getOrElse { emptyList() } }
    ...
    suspend fun browse(browseId: String, params: String?): BrowseContent = withContext(Dispatchers.IO) {
        runCatching { BrowseParser.parseBrowseContent(client.browse(browseId, params)) }
